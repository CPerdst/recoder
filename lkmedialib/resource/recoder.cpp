...
void recoder::record()
{
    if(mux_option_.grab_option().has_window()){
        ...
        [this](AVFrame* frame){
            ...
            if(err < 0){
                qDebug() << __FILE__ << " av_frame_get_buffer failed to alloc memory for frameYuv";
                exit(0);
            }
...
void recoder::consumer()
{
    ...
    if(err < 0){
        qDebug() << "avformat_alloc_output_context2 failed with: ";
        exit(0);
    }
    ...
    if(mux_option_.has_audio()){
        ...
        if(!coder_a){
            qDebug() << "avcodec_find_encoder for AV_CODEC_ID_AAC failed";
            exit(0);
        }
        coder_ctx_a = avcodec_alloc_context3(coder_a);
        if(!coder_ctx_a){
            qDebug() << "avcodec_alloc_context3 for coder_a failed";
            exit(0);
        }
        ...
        if(err < 0){
            qDebug() << "avcodec_parameters_from_context failed with: ";
            exit(0);
        }

        // 打开编码器
        if(avcodec_open2(coder_ctx_a, coder_a, nullptr) < 0){
            qDebug() << "avcodec_open2 for audio failed";
            exit(0);
        }
    ...
    if(mux_option_.grab_option().has_window()){
        ...
        if(!coder_v){
            qDebug() << "avcodec_find_encoder for AV_CODEC_ID_H264 failed";
            exit(0);
        }
        ...
        qDebug() << "coder height: " << coder_ctx_v->height << " coder width: " << coder_ctx_v->width;
        ...
        if(err < 0){
            qDebug() << "avcodec_open2 for video failed";
            exit(0);
        }
        ...
        if(err < 0){
            qDebug() << "avcodec_parameters_from_context failed for video";
            exit(0);
        }
        ...
        if(mux_option_.grab_option().has_camera()){
            ...
            coder_ctx_v->max_b_frames = 0; // Disable B-frames
            coder_ctx_v->has_b_frames = 0; // Ensure no B-frames are used

            err = avcodec_open2(coder_ctx_v, coder_v, nullptr);
            if(err < 0){
                qDebug() << "avcodec_open2 for camera failed";
                exit(0);
            }
            ...
            err = avcodec_parameters_from_context(o_stream_v->codecpar, coder_ctx_v);
            if(err < 0){
                qDebug() << "avcodec_parameters_from_context failed for camera";
                exit(0);
            }

            qDebug() << "o_stream_ctx time base: " << av_q2d(o_stream_v->time_base);
    ...
    if((err = avformat_write_header(o_fmt_ctx, nullptr)) < 0){
        qDebug() << "avformat_write_header failed with: ";
        exit(0);
    }
    ...
    while(true){
        ...
        assert(frame->format == coder_ctx_v->pix_fmt);
        assert(frame->width == coder_ctx_v->width);
        assert(frame->height == coder_ctx_v->height);
//        qDebug() << "frame pts: " << frame->pts;
        ...
        if(err < 0){
            if(err == AVERROR(EAGAIN)){
                qDebug() << "Encoder not ready, try again";
                continue;
            } else if(err == AVERROR_EOF){
                // 编码结束，退出循环
                break;
            } else {
                qDebug() << "avcodec_send_frame failed with error: ";
                exit(0);
            }
        }
        ...
        while (true) {
            ...
            if (err == AVERROR(EAGAIN) || err == AVERROR_EOF) {
                ...
                qDebug() << "avcodec_receive_packet failed with error: ";
                exit(0);
            }
...
//            qDebug() << "after trans: "<< packet.pts << " " << packet.dts << " " << packet.duration;
            ...
            qDebug() << "Encoder time base: " << coder_ctx_v->time_base.num << "/" << coder_ctx_v->time_base.den;
            qDebug() << "Stream time base: " << o_stream_v->time_base.num << "/" << o_stream_v->time_base.den;

            if((err = av_interleaved_write_frame(o_fmt_ctx, &packet)) < 0){
                qDebug() << "av_interleaved_write_frame failed with error: ";
                exit(0);
            }
